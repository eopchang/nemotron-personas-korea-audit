...
def cmi(df: pl.DataFrame, x: str, y: str, z: str | Sequence[str]) -> float:
    ...
    if total == 0:
        return 0.0
    cmi_val = 0.0
    for z_val, sub in g.groupby("__zkey__", sort=False):
        p_z = sub["n"].sum() / total
        joint = sub.pivot(index=x, columns=y, values="n").fillna(0).values.astype(np.float64)
        cmi_val += p_z * _mi_from_2d(joint)
