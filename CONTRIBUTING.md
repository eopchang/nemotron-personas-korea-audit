...
이 리포에 기여를 환영합니다. 본 프로젝트는 **데이터 감사(audit)** 이므로,
...
- **결과 반박**: 우리 결론이 틀렸음을 보이는 데이터·코드. 이게 가장 가치 있음.
- **다른 LLM/모델로 외부 리뷰**: 결과를 [`review/`](review/) 의 packet으로 점검

### 보류합니다
- 데이터셋 자체의 수정 (이건 NVIDIA의 영역)
- 정치적·이데올로기적 비판 (감사는 통계적 사실에 한정)
...
- Python 3.12, ruff/black 권장 (강제 아님)
...
서로 존중. NVIDIA 직원이 본 결과를 정정하거나 보강하면 환영합니다.
이 감사는 NVIDIA를 비판하는 것이 목적이 아니라 **이 데이터셋의 적절한 사용 범위**
