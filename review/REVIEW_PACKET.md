...
### 3-7. province × housing_type — Phase 1 신호의 추적
Phase 1에서 발견한 "전체적으로 아파트 +8.9pp 과대" 신호의 **결합 분포 내부 구조**:
...
방향성 차이가 가장 큰 페어들 — `U(Y|X)` 와 `U(X|Y)` 의 격차가 클수록 **한쪽이 다른 쪽을 강하게 함의**한다는 신호:
...
| housing ~ occupation | 0.010 | 0.010 | **0%** | military+sex (drop이 없지만 마지널 MI 0.010 nats 가 임계 0.005 의 2배 — *threshold artifact 또는 매우 약한 잔존 신호 가능성. P3 ε sensitivity 분석에서 재확인 필요*) |
