...
정밀 점검 완료 ([Phase 3 §1.3.1](../reports/PHASE3_REPORT.md#131-leakage-check--위-probe-결과는-데이터-누수에-영향받았나)). 잠재 leakage 6가지 (전체 데이터로 OrdinalEncoder fit, cap_high_card 전체 빈도 기반 등) 를 식별 후 train-only encoder + 5-fold CV 로 재실행. **6개 모든 case 에서 info_added 변화 < 0.005 nats** (원본 효과의 1% 이하). 결론 (Q1 housing decoupled, controls 모두 작동) 그대로 유지. 우려는 합리적이었으나 실제 영향은 무의미.
