...
변수 한 개의 엔트로피가 보통 0.5 ~ 2 nats 인 점을 고려해, "0.5%~ 1% 의 정보 공유" 를 효과 없는 수준으로 본 임의 임계입니다. 더 보수적인 임계 (ε = 0.01) 면 direct edge 가 더 적게, 더 관대한 임계 (ε = 0.001) 면 더 많이 나옵니다. 본 리포는 ε 를 변경하면 결과가 어떻게 바뀌는지 sensitivity 분석은 *아직* 안 했습니다 — 이슈 환영.
