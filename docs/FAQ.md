...
### Q. housing 의 TVD = 0.12 격차는 진짜 NVIDIA 의 marginal-level 실수였나요?
**부분적으로 우리 측의 단위 mismatch 환상이었습니다.** 자체 비판적 재검토 결과 ([Phase 1 §3-4 수정본](../reports/PHASE1_REPORT.md#3-4-housing_type----신호-약화됨-단위-mismatch-보정-후)):

원래 비교가 잘못됨: Nemotron 은 **개인 단위** (1M 행 = 1M 명), KOSIS reference 는 **가구 단위** (1가구 = 1행). 거처유형별 평균 가구원수 차이 때문에 두 단위는 직접 비교 불가.

자체 추정 per-person reference 로 보정 시:
- 원본 TVD = 0.119
- **보정 TVD ≈ 0.084 (약 30% 감소)**
- 아파트 격차 +8.9pp → +3.5pp (대폭 축소)
- 단독주택 격차 -11.5pp → -8.0pp (여전히 큼)

→ 원래 "housing 만 큰 격차" 클레임은 약화. 단독주택 -8pp 잔존 격차는 **약한 신호** 로만 인용. Phase 3 의 housing decoupling 결론 (sex/age/marital 무관) 은 internal structure 분석이라 이 보정과 무관.
...
정밀 점검 완료 ([Phase 3 §1.3.1](../reports/PHASE3_REPORT.md#131-leakage-check--위-probe-결과는-데이터-누수에-영향받았나)). 잠재 leakage 6가지 (전체 데이터로 OrdinalEncoder fit, cap_high_card 전체 빈도 기반 등) 를 식별 후 train-only encoder + 5-fold CV 로 재실행. **6개 모든 case 에서 info_added 변화 < 0.005 nats** (원본 신호의 1% 이하). 결론 (Q1 housing decoupled, controls 모두 작동) 그대로 유지. 우려는 합리적이었으나 실제 영향은 무의미.
