...
### LLM 학습 · 합성 데이터 다양성 확대 · 모델 편향 완화
- NVIDIA 가 데이터 카드에 명시한 본래 용도 (한국 도메인 LLM 학습, 소버린 AI 개발 포함).
- 텍스트 다양성·자연스러움 우수 — 한국어 LLM 의 "한국적 인물" 표현 능력 향상에 적합.
...
### 주거 정책 / 주거 빈곤 연구 ❌
- **이유**: housing_type 이 사람 속성과 통계적으로 분리됨 (예측가능성 검사 info_added = −0.008 nats).
